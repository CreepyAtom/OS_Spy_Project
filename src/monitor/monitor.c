...
void display_general_information_values(WINDOW *window, memory_t *mem)
{
    ...
    elapsed_time         = mem->elapsed_time/1000;
   ...
   /* ---------------------------------------------------------------------- */
    ...
    //mvwprintw(window, 20, 8, "%f", mem->elapsed_time);
    mvwprintw(window, 20, 26, "       ");
    mvwprintw(window, 20, 26, "%.2d h %.2d", hour, minutes);
    //mvwprintw(window, 20, 26, "%.2d h %.2d", mem->hour, mem->minute);
...
void display_enemy_country_monitor(WINDOW *window) {
	...
	mqd_t msg_queue = mq_open(ENEMY_MQUEUE, O_RDONLY|O_CREAT, 0666, NULL);
	if (msg_queue == -1) printf("Error when creating mqueue");

	mq_getattr(msg_queue, &attr);
	int nb_msg = 0;


	char *current_message = malloc(attr.mq_msgsize);
	unsigned int current_priority;

	sem_wait(&sem_mqueue);
	mq_receive(msg_queue, current_message, attr.mq_msgsize, &current_priority); // should wait for a message to appear
	sem_post(&sem_mqueue);

	if (nb_msg == MAX_NB_MSG) {
		// TODO faire qu'il remplace l'élément de plus faible priorité
	} else {
		if (strlen(current_message) != 0) {
			nbr_message = nbr_message + 1;
			mvwprintw(window, 2, msg_column, "[%d] messages.", nbr_message);   // Displays the number of messages

			mvwprintw(window, (nb_lines + ((nbr_message - 1) % MAX_NB_MSG)), msg_column, "[%d]\t- %s", current_priority, current_message);
		}
	}
	wrefresh(window);
